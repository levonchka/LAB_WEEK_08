...
class NotificationService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        val id = intent?.getStringExtra("Id") ?: "001"
        ...
        scope.launch {
            ...
            withContext(Dispatchers.Main) {
                Toast.makeText(applicationContext, "Channel id $id process is done!", Toast.LENGTH_SHORT).show()
    ...
    private fun createNotificationChannel(channelId: String) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val channel = NotificationChannel(channelId, "Main Service Channel", NotificationManager.IMPORTANCE_LOW)
